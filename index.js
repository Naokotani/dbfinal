...
//const db = new sqlite3.Database('database/final.db');
...
`);
  ...
  db.each("SELECT order_id, customer_id, book_id, order_date, quantity FROM orders", (err, row) => {
		console.log("Order List");
		if (err) console.error(err)
    console.log(`${row.order_id}: ${row.customer_id} ${row.book_id} ${row.order_date}, ${row.quantity}`);
  });

  db.each("SELECT author_id, author_name, birth_date, nationality FROM authors", (err, row) => {
		console.log("Author list");
		if (err) console.error(err)
    console.log(`${row.author_id}: "${row.author_name}" ${row.birth_date} ${row.nationality}`);
  });
  ...
  db.each("SELECT customer_id, first_name, last_name, email FROM customers", (err, row) => {
		console.log("Customer List");
		if (err) console.error(err)
    console.log(`${row.customer_id}: ${row.first_name} ${row.last_name} ${row.email}`);
  });
